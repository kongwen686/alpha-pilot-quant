...
function normalizeState(raw: QuantState): QuantState {
  ...
  const state = {
    ...
    quantToolbox: raw.quantToolbox ?? initial.quantToolbox,
...
const mutationRoutes: Array<{ method: string; pattern: RegExp; handler: Handler }> = [
  ...
  { method: "POST", pattern: /^\/api\/quant-toolbox\/sync$/, handler: (state) => syncQuantToolbox(state) },
...
const readRoutes: Record<string, (state: QuantState) => unknown> = {
  ...
  "/api/quant-toolbox": (state) => state.quantToolbox,
